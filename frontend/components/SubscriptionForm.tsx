...
export function SubscriptionForm({
  ...
  return (
    <form onSubmit={(e) => void submit(e)} className="space-y-3 rounded-xl border border-line bg-panel p-4">
      ...
      <label>Webhook URL<input onChange={(e) => setForm({ ...form, channel_config: { webhook_url: e.target.value } })} placeholder="https://open.feishu.cn/open-apis/bot/v2/hook/..." /></label>
      <label>LLM 配置 ID（可选）<input onChange={(e) => setForm({ ...form, llm_config_id: e.target.value || null })} placeholder="在设置页复制" /></label>
      <label>渠道配置 ID（可选）<input onChange={(e) => setForm({ ...form, channel_config_id: e.target.value || null })} placeholder="优先于上方 Webhook" /></label>
