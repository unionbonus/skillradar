...
if dock compose version >/dev/null 2>&1; then
  ...
  dock build -t skillradar-backend:0.5.3 "$APP/backend"
  dock build --build-arg BACKEND_URL=http://backend:8000 -t skillradar-frontend:0.5.3 "$APP/frontend"
  ...
  if ! dock run -d --name skillradar-frontend --restart unless-stopped \
    ...
    skillradar-frontend:0.5.3; then
