...
---
*由 SkillRadar v0.5 自动生成，可在工作台编辑后导出。*
"""
...
def llm_complete(
    ...
    if not key:
        raise AnalysisError("LLM_API_KEY not configured")
    url = (base_url or settings.llm_base_url).rstrip("/") + "/chat/completions"
    headers = {"Authorization": f"Bearer {key}", "Content-Type": "application/json"}
    body = {
        "model": model or settings.llm_model,
        "messages": [
            {"role": "system", "content": "You are SkillRadar analyst. Reply with valid JSON only."},
    ...
    try:
        ...
        if not isinstance(content, str) or not content.strip():
            raise AnalysisError("empty LLM content")
        return content
    except httpx.HTTPError as exc:
        raise AnalysisError(f"LLM request failed: {exc}") from exc
    except (KeyError, IndexError, TypeError, ValueError) as exc:
        raise AnalysisError(f"LLM response malformed: {exc}") from exc
