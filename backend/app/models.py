...
class Subscription(Base):
    ...
    llm_config_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("llm_configs.id"), nullable=True)
    channel_config_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("channel_configs.id"), nullable=True)
...
class ChannelConfig(Base):
    ...
    user_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True, index=True)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    channel_type: Mapped[str] = mapped_column(String(20), default="feishu")
...
class CommercialReport(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    repository_id: Mapped[int] = mapped_column(ForeignKey("repositories.id"), nullable=False, unique=True, index=True)
    market_research_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)
    content_md: Mapped[str] = mapped_column(Text, default="")
    content_json: Mapped[dict] = mapped_column(JSON, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, onupdate=utcnow)
